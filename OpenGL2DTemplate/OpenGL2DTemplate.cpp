...
// Function to draw fireworks at a given location
void DrawFirework(float x, float y) {
    glPointSize(3.0);
    glBegin(GL_POINTS);
    for (int i = 0; i < 100; i++) {
        float angle = 2 * PI * rand() / RAND_MAX;
        float radius = 2.0 * rand() / RAND_MAX;
...
void Display() {
    ...
    // Draw multiple fireworks at different locations
    for (int i = 0; i < 100; i++) {
        float x = rand() % 800;
        float y = rand() % 600;
        DrawFirework(x, y);
    ...
    if (loose) {
        ...
        glPushMatrix();
        glRasterPos2i(width / 2 - 20, height / 2);
